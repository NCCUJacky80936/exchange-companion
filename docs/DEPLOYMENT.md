...
Resolve the hosting route before running any provider command:

1. If `.openai/hosting.json` exists and contains a non-empty `project_id`, the existing Codex Sites project is the canonical production target. Reuse that binding with the Sites building/hosting workflow for normal releases.
2. If there is no Sites project binding and the environment provides Sites hosting, use Sites to create or select the user-owned site according to the Sites skill.
3. Use Cloudflare Workers only when the user explicitly requests a Worker migration or a separate Worker deployment. The repository guard requires `ALLOW_CLOUDFLARE_WORKERS_DEPLOY=1` for that one command; do not persist the override.

Do not run Wrangler login, Wrangler preflight, or the Cloudflare deploy scripts for an ordinary update while a Sites binding exists. Installing Cloudflare agent skills or MCP servers is a separate tooling setup and does not change this routing precedence.
...
## Cloudflare Workers (explicit exception only)

This is not the normal production path when `.openai/hosting.json` contains a Sites binding. Only use it for an explicit Worker deployment or migration request.
...
```bash
ALLOW_CLOUDFLARE_WORKERS_DEPLOY=1 npm run deploy:preflight
ALLOW_CLOUDFLARE_WORKERS_DEPLOY=1 npm run deploy:cloudflare
```
