...
瀏覽 `http://localhost:3000/`。此時不需要帳號、雲端或 AI 額度。

## 4. 讓 Codex 補齊內容

在 repository 內要求 Codex 使用 `$create-exchange-companion`。Codex 會先確認你願意授權的資料範圍，再使用 `$exchange-concierge` 產生可匯入的提案。提案不會自動套用。
