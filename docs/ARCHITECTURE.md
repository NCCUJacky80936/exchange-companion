...
- `app/lib/cloud.ts`：選配 Supabase 私人同步、旅行分享與 Telegram 配對狀態管理。
- `app/lib/types.ts`：網站與 import bundle 的共用 entity 定義。
- `supabase/migrations/`：包含私人手帳狀態、可撤銷連線、提案紀錄，以及 Telegram 配對碼、連結與訊息安全佇列（具備 RLS 與 14 天定期清理）。

## 自動化層

- `$create-exchange-companion`：完整製作／部署 orchestrator。
- `$exchange-concierge`：證據與 current-state reconciliation，支援 `--include-telegram` 佇列提取與批次整理。
- `$exchange-email-intake`：只讀目前使用者明確授權的信件範圍，將去敏證據交給 Concierge。
- `telegram-concierge-webhook`：Supabase Edge Function，負責驗證 Telegram 來源密鑰、限定私人純文字聊天、支援指令並將訊息安全入列。
- `exchange-concierge-sync`：Supabase Edge Function，負責手帳狀態同步、Agent 提案提交與 Telegram 佇列原子領取（claim）。
- `scripts/`：初始化、profile、Skill、隱私與環境檢查。

公開網站不內建固定付費 AI 呼叫。Codex / Agent 在使用者授權的工作環境完成研究與整理，網站與 Telegram Bot 只作為安全收件與收件匣提案呈現，避免把私密來源或例行 token 成本綁進公開產品。
