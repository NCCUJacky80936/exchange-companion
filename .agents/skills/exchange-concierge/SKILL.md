...
4. Create a deterministic evidence inventory before deciding what is new. Read [evidence-coverage.md](references/evidence-coverage.md), search the authorized project files and existing private email archives by category, then search connected mail for gaps. A handoff is current website state, not a complete evidence index; `0 new emails` never means `0 usable evidence`.
   ...
   Within a selected leisure trip, use `travel-plan.stays` for hotel bases and `travel-plan.references` for its Maps lists, Sheets, guides, and booking reference pages. Preserve the user's existing daily itinerary unless they explicitly ask the Agent to schedule it.
...
- The website owns the processing timestamp and automatically removes a processed URL intake record 48 hours after the user applies that proposal. Agents set only `status: processed`; they must not author retention timestamps or delete pending intake records.
