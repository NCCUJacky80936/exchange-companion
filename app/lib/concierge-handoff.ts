...
const editableSurfaces: HandoffSurface[] = [
  ...
  { id: "resources", statePath: "state.resources[]", proposalEntity: "resource", privacy: "mixed", fields: ["title", "description", "details", "category", "type", "url", "verifiedAt", "region", "origin", "privacy", "sourceLabel", "searchTags"], notes: "私人上傳衍生資料保持 private；去識別的官方公共資源才可 shareable。description 與 details 要可獨立理解，searchTags 放常見同義詞供純文字搜尋且不顯示於卡片。" },
  { id: "resource-intake", statePath: "state.resourceIntake[]", proposalEntity: "resource-intake", privacy: "private", fields: ["url", "note", "status", "createdAt", "intent", "targetTravelPlanId"], notes: "pending URL 只授權處理該網址，不授權擴大掃描；intent: travel-import 時只能對 targetTravelPlanId 提出旅行更新。Agent 只提出 status: processed；processedAt 與套用後 48 小時清除由網站管理。" },
  ...
  { id: "study-events", statePath: "state.studyEvents[]", proposalEntity: "study-event", privacy: "private", fields: ["title", "kind", "startDate", "endDate", "startTime", "endTime", "location", "repeatWeekly", "mandatory", "notes"], notes: "供旅行衝突檢查使用，不納入公開分享。" },
  { id: "travel-plans", statePath: "state.travelPlans[]", proposalEntity: "travel-plan", privacy: "mixed", fields: ["title", "destinations", "startDate", "endDate", "travelers", "budget", "currency", "notes", "stays[]", "references[]", "days[].activities[].time", "days[].activities[].location", "days[].activities[].mapsUrl", "days[].activities[].durationMinutes", "days[].activities[].cost", "days[].activities[].booked", "days[].activities[].imageUrl", "days[].activities[].imageAlt", "days[].activities[].imageSourceLabel", "days[].activities[].imageSourceUrl", "travelNotes[]", "packingItems[]"], notes: "支援空白開始與未來多國旅行；外部圖片只保留一般 HTTP(S) 網址、替代文字與來源，不得內嵌或重新託管。分享前仍需由使用者選定單一旅程與權限。" },
];
