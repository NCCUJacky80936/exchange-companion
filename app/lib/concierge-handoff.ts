...
const editableSurfaces: HandoffSurface[] = [
  ...
  { id: "resource-intake", statePath: "state.resourceIntake[]", proposalEntity: "resource-intake", privacy: "private", fields: ["url", "note", "status", "createdAt"], notes: "pending URL 只授權處理該網址，不授權擴大掃描；完成前必須交叉檢查任務、資源、預算、行李、航班、課程與旅行欄位，不能只建立書籤。Agent 只提出 status: processed；processedAt 與套用後 48 小時清除由網站管理。" },
  ...
  { id: "travel-plans", statePath: "state.travelPlans[]", proposalEntity: "travel-plan", privacy: "mixed", fields: ["title", "destinations", "startDate", "endDate", "travelers", "budget", "currency", "notes", "stays[]", "references[]", "days[].activities[].time", "days[].activities[].location", "days[].activities[].mapsUrl", "days[].activities[].durationMinutes", "days[].activities[].cost", "days[].activities[].booked", "travelNotes[]", "packingItems[]"], notes: "支援空白開始與未來多國旅行；住宿基地與該趟旅行的 Maps／Sheet 參考資料可獨立更新，分享前仍需由使用者選定單一旅程與權限。" },
];
