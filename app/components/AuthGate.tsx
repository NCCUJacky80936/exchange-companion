...
import { ArrowLeft, ArrowRight, Bot, GitFork, LockKeyhole, MapPinned, Plane, RefreshCw, Sparkles, UserRound } from "lucide-react";
  ...
  if (view === "welcome") return (
    <main className="welcome-shell">
      ...
      <section className="welcome-hero">
        <div className="welcome-copy"><p className="eyebrow">Your private exchange command center</p><h1>交換很複雜，<br />手帳不用。</h1><p>把學校信件、行前任務、預算、行李與旅行安排放在同一個網頁 App。AI Agent 幫你整理，你只需要確認。</p><div className="welcome-actions"><button className="button primary" onClick={() => setView("login")}>登入開始使用<ArrowRight size={18} /></button><a className="button secondary" href="https://github.com/NCCUJacky80936/exchange-companion" target="_blank" rel="noreferrer"><GitFork size={18} />用 GitHub 建立自己的版本</a></div><small><LockKeyhole size={14} />需要帳號，是因為交換進度、信件摘要、預算與行政資料都屬於私人紀錄；這是可安裝的網頁 App，不是 App Store 應用程式。</small></div>
        <div className="welcome-art"><Image src="/images/exchange-hero-placeholder.svg" alt="手繪交換旅行行李與路線" width={520} height={390} /></div>
      </section>
      <section className="welcome-features" aria-label="主要功能"><article><MapPinned /><h2>一條交換路線</h2><p>從錄取、簽證、住宿到返國，期限與阻塞狀態集中追蹤。</p></article><article><Bot /><h2>AI 幫你補齊</h2><p>讀取你授權的信件、文件與網址，產生可逐項審核的更新。</p></article><article><Plane /><h2>行李與旅行一起看</h2><p>核對本人機票額度，並檢查旅行是否撞到上課或考試。</p></article></section>
    </main>
  );
  ...
  return (
    <main className="auth-shell">
      <section className="auth-card paper-card" aria-labelledby="auth-title"><span className="tape" /><button className="auth-back" onClick={() => setView("welcome")}><ArrowLeft size={16} />回到介紹</button><div className="auth-brand"><span className="brand-stamp">旅</span><div><strong>交換手帳</strong><small>{view === "login" ? "Welcome back" : "Create your private notebook"}</small></div></div>
        <div className="auth-grid"><div className="auth-copy"><p className="eyebrow">{view === "login" ? "Private sign in" : "Step 1 of 3"}</p><h1 id="auth-title">{view === "login" ? "打開你的私人手帳" : "先建立登入資料"}</h1><p>{view === "login" ? "登入後才會載入你的交換進度與 AI 整理結果。" : "帳號資料完成後，再用兩個短步驟設定交換目的地；全部走完才會進入主畫面。"}</p></div><div className="auth-art" aria-hidden="true"><Image src="/images/doodle-icons/passport-safe.png" alt="" width={210} height={210} /></div></div>
