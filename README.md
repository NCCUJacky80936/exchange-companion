...
```mermaid
flowchart LR
  A["網站下載目前進度交接檔"] --> B["把檔案交給 Codex 並明確授權來源"]
  B --> C["Email Intake 依授權範圍擷取；Concierge 比對檔案與最新官方資料"]
  C --> D["產生跨旅程、任務、預算、資源、行李、課程與旅行的待審提案"]
  D --> E["回到網站匯入，逐項或批次套用、忽略、復原"]
  E --> F["登入帳號後自動同步私人手帳"]
```

AI 頁面的「交給 Exchange Concierge 整理」會下載自我說明的私人交接 JSON，內含目前完整狀態、每個可更新頁面的欄位、Skill 入口，以及第一次建站設定的鎖定快照。日常整理可更新任務、基礎預算、資源、行李、課表與旅行，但不會重做國家、幣別、時區、固定文案或圖片。自動化不會靜默覆蓋手動修改；完整信件、附件與證件也不會匯入網站。
